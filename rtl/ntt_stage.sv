...
module ntt_stage #(
        ...
        if (DPND_FUT_DATA == 1) begin
            for (genvar gv_i = 0; gv_i < NUM_COEFS_PER_STAGE; gv_i = gv_i+2) begin: fifo_in_mux
                assign seleced_input_for_fifo[gv_i] =
                    ((STAGE_INDEX == 0) ? !R_fifo_mux_sel : R_fifo_mux_sel)
                        ? input_poly.coefs[gv_i]
                        : input_poly.coefs[gv_i+1];
            ...
            for (genvar gv_j = 0; gv_j < NUM_BUTFLY_PER_STAGE * 2; gv_j = gv_j+1) begin: butterfly_mux
                assign seleced_input_for_butterfly[gv_j] =
                    R_ripple_fifo[gv_j].in_fifo[DELAY_NUM_CLOCKS-1];
            end
        end
    endgenerate
    ...
    generate
        if (FWD_INV == 0) begin: generate_w_on_cnt_reset
            ...
            if ((STAGE_INDEX == 0 && NUM_BUTFLY_PER_STAGE > 1)
             ...
             || (STAGE_INDEX == 2 && NUM_BUTFLY_PER_STAGE > 4)) begin
    ...
    always @(posedge clk) begin
        if (reset_n == 1'b0) begin
            ...
            if (R_fifo_cnt == DELAY_NUM_CLOCKS-1 && input_poly.valid) begin
                R_fifo_mux_sel <= (STAGE_INDEX == 0 || (DPND_FUT_DATA == 0))
                    ? 1
                    : !R_fifo_mux_sel;
            end
        end
